    ...
    try {
      ...
      const insertPromises = promos.map(promo => {
        ...
        return connection.query<ResultSetHeader>(
          `INSERT INTO promotions (nom, prix, anciennePromo, source)
          VALUES (?, ?, ?, ?)`,
          [promo.nom, promo.prix, promo.anciennePromo, source],
        );
      });
      ...
      console.error("Erreur lors de l'insertion des promotions dans la base de données", error);
